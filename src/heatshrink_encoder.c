 ...
 * the lookback portion of into the encoder buffer (IE The part that is searched
 * for back references.
 *
 ...
 * directly on a file thus, avoiding the buffer copy. Since this operation
 * is done on the internal heatshrink encoder structure it should be
 * located inside the heatshrink encoder itself but that would be modifying
 * the library and would require a pull request.
 *
